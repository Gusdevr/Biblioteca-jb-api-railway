...
function carregarEmprestimos(usuarioId) {
    fetch(`http://localhost:3750/usuario/${usuarioId}/emprestimos`)
        .then(res => res.json())
        .then(emprestimos => {
...
function devolverLivro(emprestimoId) {
    ...
    fetch(`http://localhost:3750/devolver/${emprestimoId}`, {  // Alterado para DELETE e passou emprestimoId na URL
        method: 'DELETE',
        headers: {
...
function alugarLivro(livroId) {
    ...
    fetch('http://localhost:3750/alugar', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
